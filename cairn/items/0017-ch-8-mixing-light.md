...
**The argument.** Interpolation is the operation programs perform most often on colour and get wrong most often. The fix is not a better formula; it is noticing that `lerp` needs a space argument and that the default choice is the worst available one.

6 sections · 7,400 planned words · part III
Outline: `outline/ch08.outline.ts`

**Figures.** `interpolation-spaces`

**Sources.** `css-color-4`, `porter-duff-1984`, `poynton-video`
